...
to the NeSy4VRD OWL ontology, VRD-World.

This script does the following:
* it instantiates an empty RDFLib knowledge graph (KG)
...
* it uses RDFLib's companion Python package OWLRL to do the OWL reasoning, to 
  logically expand the scene graph annotated for each image, inferring new
  visual relationships using common sense logical inference
* it uses a SPARQL query to extract the logically expanded scene graph for
  each image from the KG
* it converts the extracted scene graph for each image back into their native 
  NeSy4VRD format
* it saves the reconstituted (now logically expanded) NeSy4VRD image scene graphs 
...
# get the set of NeSy4VRD visual relationship annotations we want to augment
#vrd_annotations_path = os.path.join(anno_dir, 'nesy4vrd_annotations_test.json')
...
anno_augmented_dir = os.path.join('~', 'research', 'loadAndAugment')
anno_augmented_dir = os.path.expanduser(anno_augmented_dir)
...
if redirect_stdout:
    log_filename = 'load_and_augment_2_log.txt'
    stdout_file_saved = sys.stdout
    file_path_name = os.path.join(anno_augmented_dir, log_filename)
    print(f'redirecting stdout to log file: {file_path_name}')
    sys.stdout = open(file_path_name, 'w')

print()
print('*** load_and_augment_2.py log file ***')
print()
print(f'output directory: {anno_augmented_dir}')
...
ontology_load_time = end_time - start_time
...
annotated_vr_load_time = end_time - start_time
...
materialisation_time = end_time - start_time
...
ontology_load_time_minutes = ontology_load_time / 60

annotated_vr_load_time_minutes = annotated_vr_load_time / 60

materialisation_time_minutes = materialisation_time / 60

total_time = ontology_load_time + annotated_vr_load_time + materialisation_time

total_load_and_infer_time_minutes = total_time / 60

print()
print('------------------------------------------------------------------')
print()
print('Reporting load and inference timings ...')
print()
print(f'Ontology load time: {ontology_load_time_minutes:.2f} minutes')
print()
print(f'Annotated VR load time: {annotated_vr_load_time_minutes:.2f} minutes')
print()
print(f'Materialisation time: {materialisation_time_minutes:.2f} minutes')
print()
print(f"Combined load and inference time: {total_load_and_infer_time_minutes:.2f} minutes")
print()
...
for idx, imname in enumerate(vrd_img_names): 
    ...
    for row in qres:
        ...
        if row.subObj == row.objObj:
            raise ValueError(f'subObj same as objObj on image {imname}')
...
filename = 'nesy4vrd_annotations_train_augmented_per_onto_v1_2c.json'
