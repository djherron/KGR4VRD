...
'''
This script summarises image-level scores of metrics relating to the analysis
of predicted VRs wrt symmetric pairs.  It creates an output .csv file of
consolidated (predicted VR) cell results analyse from the perspective of
symmetric pairs.

The script processes 1 cell at a time. Each cell has results for 12 models:
3 topN values * 4 job runs. The output .csv file has summary symmetric pair
...
for idx in range(nrows):
    ...
    # build the name of the target symmetric pairs predictions .csv file
    # to be summarised;
    # example: vrd_ppnn_trc0460_ckpt_034_prc018_preds_symmetric_pairs_rrc01.csv
    # (nb: this file is cell specific, job run specific and epoch specific)
    sym_pairs_filename = 'vrd_ppnn_' + trc_id + '_ckpt_' + cell_run_epoch_str + \
                           '_' + prc_id + '_preds_symmetric_pairs_' + rrc_id + '.csv'
    
    # assemble the file path for the target symmetric pairs .csv file
    filepath = os.path.join(predictions_dir_base, cell_folder, 
                            cell_run_folder, sym_pairs_filename)
    
    print(filepath)
    
    # get the image-level scores of our symmetric pairs metrics
    res_df = pd.read_csv(filepath)

print()
print('all 12 symmetric pairs .csv files can be found and loaded')
...
# symmetric pair metric scores (means)
n_sp_pvrs = []
n_sp_pvr_pairs = []
n_sp_pvr_singletons = []
proportion_sp_pvrs_in_pairs = []
...
n_sp_pvrs_hits_topN_025 = []
n_sp_pvrs_hits_topN_050 = []
n_sp_pvrs_hits_topN_100 = []
n_sp_pvrs_hits_topN_999 = []
proportion_sp_pvrs_hits_topN_025 = []
proportion_sp_pvrs_hits_topN_050 = []
proportion_sp_pvrs_hits_topN_100 = []
proportion_sp_pvrs_hits_topN_999 = []
mean_sp_pvr_confidence = []
...
for idx in range(nrows):
    ...
    # build the name of the target symmetric pairs predictions .csv file
    # to be summarised;
    # example: vrd_ppnn_trc0460_ckpt_034_prc018_preds_symmetric_pairs.csv
    # (nb: this file is cell specific, job run specific and epoch specific)
    sym_pairs_filename = 'vrd_ppnn_' + trc_id + '_ckpt_' + cell_run_epoch_str + \
                           '_' + prc_id + '_preds_symmetric_pairs_' + rrc_id + '.csv'
    
    print(f"Symmetric pairs .csv filename: {sym_pairs_filename}")
    ...
    mean_n_sp_pvrs = np.round(res_df['n_sp_pvrs'].mean(), n_decimal)
    mean_n_sp_pvr_pairs = np.round(res_df['n_sp_pvr_pairs'].mean(), n_decimal)
    mean_n_sp_pvr_singletons = np.round(res_df['n_sp_pvr_singletons'].mean(), n_decimal)
    mean_prop_sp_pvrs_in_pairs = np.round(res_df['proportion_sp_pvrs_in_pairs'].mean(), n_decimal)
    ...
    mean_n_sp_pvrs_hits_topN_025 = np.round(res_df['n_sp_pvrs_hits_topN_025'].mean(), n_decimal)
    mean_n_sp_pvrs_hits_topN_050 = np.round(res_df['n_sp_pvrs_hits_topN_050'].mean(), n_decimal)
    mean_n_sp_pvrs_hits_topN_100 = np.round(res_df['n_sp_pvrs_hits_topN_100'].mean(), n_decimal)
    mean_n_sp_pvrs_hits_topN_999 = np.round(res_df['n_sp_pvrs_hits_topN_999'].mean(), n_decimal)
    mean_prop_sp_pvrs_hits_topN_025 = np.round(res_df['proportion_sp_pvrs_hits_topN_025'].mean(), n_decimal)
    mean_prop_sp_pvrs_hits_topN_050 = np.round(res_df['proportion_sp_pvrs_hits_topN_050'].mean(), n_decimal)
    mean_prop_sp_pvrs_hits_topN_100 = np.round(res_df['proportion_sp_pvrs_hits_topN_100'].mean(), n_decimal)
    mean_prop_sp_pvrs_hits_topN_999 = np.round(res_df['proportion_sp_pvrs_hits_topN_999'].mean(), n_decimal)
    mean_mean_sp_pvr_confidence = np.round(res_df['mean_sp_pvr_confidence'].mean(), n_decimal)

    # save the mean metric scores for the current cell/run/epoch combination
    n_sp_pvrs.append(mean_n_sp_pvrs)
    n_sp_pvr_pairs.append(mean_n_sp_pvr_pairs)
    n_sp_pvr_singletons.append(mean_n_sp_pvr_singletons)
    proportion_sp_pvrs_in_pairs.append(mean_prop_sp_pvrs_in_pairs)
    ...
    n_sp_pvrs_hits_topN_025.append(mean_n_sp_pvrs_hits_topN_025)
    n_sp_pvrs_hits_topN_050.append(mean_n_sp_pvrs_hits_topN_050)
    n_sp_pvrs_hits_topN_100.append(mean_n_sp_pvrs_hits_topN_100)
    n_sp_pvrs_hits_topN_999.append(mean_n_sp_pvrs_hits_topN_999)
    proportion_sp_pvrs_hits_topN_025.append(mean_prop_sp_pvrs_hits_topN_025)
    proportion_sp_pvrs_hits_topN_050.append(mean_prop_sp_pvrs_hits_topN_050)
    proportion_sp_pvrs_hits_topN_100.append(mean_prop_sp_pvrs_hits_topN_100)
    proportion_sp_pvrs_hits_topN_999.append(mean_prop_sp_pvrs_hits_topN_999)
    mean_sp_pvr_confidence.append(mean_mean_sp_pvr_confidence)   


#%%

# gather the summary (mean) symmetric pair metric scores into a dataframe
df = pd.DataFrame({'expt_cell_run_folder': experiment_cell_run_folders,
                   ...
                   'n_sp_pvrs_hits_topN_025': n_sp_pvrs_hits_topN_025,
                   'n_sp_pvrs_hits_topN_050': n_sp_pvrs_hits_topN_050,
                   'n_sp_pvrs_hits_topN_100': n_sp_pvrs_hits_topN_100,
                   'n_sp_pvrs_hits_topN_999': n_sp_pvrs_hits_topN_999,
                   'proportion_sp_pvrs_hits_topN_025': proportion_sp_pvrs_hits_topN_025,
                   'proportion_sp_pvrs_hits_topN_050': proportion_sp_pvrs_hits_topN_050, 
                   'proportion_sp_pvrs_hits_topN_100': proportion_sp_pvrs_hits_topN_100,
                   'proportion_sp_pvrs_hits_topN_999': proportion_sp_pvrs_hits_topN_999,
...
print()
print("Symmetric pairs consolidated results .csv file saved")
print(filepath)
